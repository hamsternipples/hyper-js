...
// the opposite of an obv, it just fires once then clears the listener
export const once = (initial) => {
  const obv = (val, do_immediately) => {
    return (
      is_void(val) ? obv.v  // getter
    : not_fn(val) ? (       // is a set
      is(obv.v, val) ? undefined : // only set if obv.v === val (but use Object.is instead of ===, so it gets NaN and +0/-0)
        ...
        obv.l = [], // reset the listeners array
        val // return the value
      )
    : ( // is a listener function
        obv.l.push(val),
  ...
  if (is_obv(initial)) {
    // cheap hack: the remove function is passed into our own liseners
    //             so it'll remove itself when it fires.
    // I set do_immediately to falsy value so it does't fire right away/
    obv.l.push(initial(obv, 0))
    // then, I steal it's current value
    obv.v = initial.v
  } else {
    obv.v = initial
  }
